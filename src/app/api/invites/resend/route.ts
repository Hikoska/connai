...
export async function POST(req: Request) {
  try {
    ...
    const iv = await sbGet('interviews', {
      token: `eq.${token}`,
      select: 'id,token,stakeholder_name,stakeholder_email,status',
      limit: '1',
    })
