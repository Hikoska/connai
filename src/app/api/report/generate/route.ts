...
Return only valid JSON, no explanation.`
  ...
  // Send report-ready email to lead
  try {
    ...
    if (process.env.RESEND_API_KEY && leadEmail) {
      await resend.emails.send({
        ...
        html: `<p>Hi,</p><p>Your Digital Maturity Assessment is complete. Overall score: <strong>${overallScore}/100</strong>.</p><p><a href="${process.env.NEXT_PUBLIC_APP_URL ?? ''}/report/${interview.lead_id}">View your report</a></p>`,
      })
    }
  } catch (emailErr) {
