...
/** Best-effort PATCH — tracking must never block report generation. */
...
export async function POST(req: Request) {
  ...
  let ivList: Record<string, string>[] = []
  ...
  // --- CL-28: insert report_run tracking row (best-effort, non-fatal) ---
  try {
    const runRows = await dbPost('report_runs', {
      lead_id:      resolvedLeadId,
      triggered_by: lead_id ? 'auto' : 'manual',
      status:       'pending',
    })
    runId = runRows[0]?.id ?? null
  } catch {
    // Tracking must never block report generation
  }
  // -------------------------------------------------------------------
  ...
  async function tryScore(model: ReturnType<typeof groq>) {
    const result = await generateText({ model, prompt: scoringPrompt, maxTokens: 512 })
  ...
  try {
    ...
    if (isRateLimit(err) && process.env.CEREBRAS_API_KEY) {
      try {
        ...
        if (runId) await dbPatch('report_runs', `id=eq.${runId}`, {
          status:       'failed',
          completed_at: new Date().toISOString(),
          duration_ms:  Date.now() - runStart,
          error_msg:    `LLM rate-limited: ${String(err2)}`,
        })
      ...
      if (runId) await dbPatch('report_runs', `id=eq.${runId}`, {
        ...
        completed_at: new Date().toISOString(),
        duration_ms:  Date.now() - runStart,
        error_msg:    `LLM error: ${String(err)}`,
      })
  ...
  const reportId = reportRows[0]?.id ?? resolvedLeadId

  // --- CL-28: mark run completed (best-effort) ---
  if (runId) {
    await dbPatch('report_runs', `id=eq.${runId}`, {
      status:            'completed',
      completed_at:      new Date().toISOString(),
      duration_ms:       Date.now() - runStart,
