...
3. **No module-level `createClient()` in API routes** — use `fetch()` to Supabase REST instead:
   ```ts
   // WRONG (breaks build):
   const supabase = createClient(url, key)
   
   // CORRECT:
   fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/table`, {
...
7. **Supabase import in client components**: `import { supabase } from '@/lib/supabase/client'` (not direct `@supabase/supabase-js` at module level — use lazy import or `@/lib/supabase/client`)
...
See `ux-sprint-brief.md` for full brief (committed to Hikoska/connai workspace).
